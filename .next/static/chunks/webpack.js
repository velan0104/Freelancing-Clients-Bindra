...
/******/ (() => {
  ...
  /******/ (() => {
    /******/ __webpack_require__.h = () => "d3f2932602219a8d";
    /******/
  })();
  /******/
  ...
  /******/ (() => {
    ...
    function validateUrl(url) {
      try {
        const parsedUrl = new URL(url, document.baseURI); // Parse the URL
        // Allow only specific trusted origins or paths
        const allowedOrigins = ["https://trusted-origin.com", location.origin];
        if (!allowedOrigins.includes(parsedUrl.origin)) {
          throw new Error("URL origin is not trusted.");
        }
    ...
    function onScriptComplete(url, script, timeout, event) {
      // Avoid memory leaks
    ...
    __webpack_require__.l = (url, done, key, chunkId) => {
      ...
      if (!script) {
        ...
        const validatedUrl = validateUrl(url); // Validate the URL
        if (!validatedUrl) {
          console.error("Script loading aborted due to invalid URL:", url);
          return;
        }
        script.src = validatedUrl; // Use the sanitized URL
      }
      ...
      const timeout = setTimeout(
        () =>
          onScriptComplete(url, script, timeout, {
            type: "timeout",
            target: script,
          }),
        120000
      ...
      script.onerror = (event) => onScriptComplete(url, script, timeout, event);
      script.onload = (event) => onScriptComplete(url, script, timeout, event);
